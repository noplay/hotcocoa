...
class DemoApplication
  ...
  class << self
    ...
    attr_reader :view_classes

    def register view_class
      view_classes << view_class
    end

    def view_with_description description
      view_classes.find { |view| view.description == description }
    end
  ...
  attr_reader :current_demo_view
  attr_reader :main_window
  attr_reader :segment_control
  ...
  def start
    ...
    application name: 'Demo' do |app|
      ...
      @main_window = window(frame: [100, 100, 600, 500], title: 'HotCocoa Demo Application' do |win|
        ...
        win << (@segment_control ||= create_segment_control)
  ...
  def add_status_bar_speech_menu_exammple
    m = menu do |main|
      main.item :speak, on_action: proc { speech_synthesizer.speak('I have a lot to say.') }
    end
  ...
  def create_segment_control
    segmented_control(constraint: {expand: :width, align: :center, start: false}, segments: demo_app_segments) do |seg|
      seg.on_action do
  ...
  def create_window_geometry_label
    label(text: '', constraint: {expand: :width, start: false}, font: font(system: 15))
  end
